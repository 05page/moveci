...
const recupererMesTransactions = async (): Promise<TransactionConclue[]> => {
  const reponse = await api.get<{ data: TransactionConclue[] }>("transactions-conclues/mes-demandes");
  return reponse.data;
};

/** Même contrat que POST /transactions-conclues/{id}/confirmer-client, body { code }. */
const confirmerTransaction = async (id: string, code: string): Promise<void> => {
  await api.post(`transactions-conclues/${id}/confirmer-client`, { code });
};

/** Même contrat que POST /transactions-conclues/{id}/refuser (PAS "refuser-client" — vérifie routes/api.php:212). */
const refuserTransaction = async (id: string): Promise<void> => {
  await api.post(`transactions-conclues/${id}/refuser`);
};

/** Même contrat que POST /transactions-conclues/{id}/restituer-client, body { code }. */
const restituerTransaction = async (id: string, code: string): Promise<void> => {
  await api.post(`transactions-conclues/${id}/restituer-client`, { code });
};
...
const PageMesTransactionsClient = () => {
  ...
  /** Refetch après mutation plutôt que patch manuel du state : même pattern que vendeur/transactions/page.tsx. */
  const confirmer = (id: string, code: string, type: TransactionConclue["type"]) => {
    setErreur(null);
    setIdEnCours(id);

    confirmerTransaction(id, code)
      .then(() => recupererMesTransactions())
      .then((liste) => {
        setTransactions(liste);
        setIdEnCours(null);
        toast.success(type === "vente" ? "Vente confirmée !" : "Location confirmée !");
      })
      .catch((erreurCatch) => {
        setIdEnCours(null);
        const message = erreurCatch?.message ?? "La confirmation a échoué.";
        setErreur(message);
        toast.error(message);
      });
  };
  ...
  const restituer = (id: string, code: string) => {
    setErreur(null);
    setIdEnCours(id);

    restituerTransaction(id, code)
      .then(() => recupererMesTransactions())
      .then((liste) => {
        setTransactions(liste);
        setIdEnCours(null);
      ...
      .catch((erreurCatch) => {
        setIdEnCours(null);
        const message = erreurCatch?.message ?? "La confirmation de restitution a échoué.";
        setErreur(message);
        toast.error(message);
      });
  };
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-5 py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Vos achats et locations conclus, confirmés ou en attente.
      </p>
      ...
      {transactions === undefined ? (
        ...
        <section className="mt-6 rounded-2xl border border-border p-12 text-center">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm text-muted-foreground">
            Une transaction se crée quand un rendez-vous aboutit à un achat ou une location.
          </p>
        </section>
      ) : (
        <>
          {enAttente.length > 0 && (
            <section className="mt-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Le code à saisir a été envoyé dans vos notifications à la fin du rendez-vous.
              </p>
              <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {enAttente.map((transaction) => (
                  <CarteTransaction
                    ...
                    perspective="client"
                    hrefDetail={`/client/transactions/${transaction.id}`}
                    onConfirmer={(code) => confirmer(transaction.id, code, transaction.type)}
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {confirmees.map((transaction) => (
                  <CarteTransaction
                    ...
                    perspective="client"
                    hrefDetail={`/client/transactions/${transaction.id}`}
                    onRestituer={(code) => restituer(transaction.id, code)}
                    enCours={idEnCours === transaction.id}
                  />
                ))}
              </div>
...
export default PageMesTransactionsClient;
